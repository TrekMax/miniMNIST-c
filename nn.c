...
typedef struct {
    float *weights;         // Flattened 2D array: weights[input_size * output_size]
    float *biases;          // 1D array: biases[output_size]
    float *weight_momentum; // Flattened 2D array: weight_momentum[input_size * output_size]
...
void mnist_read_images(const char *filename, unsigned char **images, int *nImages)
{
...
void mnist_read_labels(const char *filename, unsigned char **labels, int *nLabels)
{
    FILE *file = fopen(filename, "rb");
    if (!file) {
        exit(1);
...
int main(int argc, char *argv[])
{
    ...
    mnist_read_images(TRAIN_IMG_PATH, &data.images, &data.nImages);
    mnist_read_labels(TRAIN_LBL_PATH, &data.labels, &data.nImages);
    ...
    for (int epoch = 0; epoch < EPOCHS; epoch++) {
        ...
        printf("Epoch %3d, Accuracy: %.2f%%, Avg Loss: %.4f, Time: %.2f seconds\n", epoch + 1,
               (float)correct / test_size * 100, total_loss / train_size, cpu_time_used);
    }
